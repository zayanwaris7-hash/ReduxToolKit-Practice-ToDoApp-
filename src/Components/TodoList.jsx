...
import { DeletTodo, IsToggle, UpdateTodo } from '../Redux/TodoSlice';
...
function TodoList({ todo }) {
    ...
    return (
        <>
           {showPopup && <PopUp message={todo.message} />}
            <div className={`group flex items-center p-4 rounded-xl border transition-all duration-500 ${todo.isChecked
                ...
                <button
                    onClick={istoggleCompleted}
                    className={`w-5 h-5 rounded-md border-2 transition-all flex items-center justify-center ${todo.isChecked
                ...
                <input
                    type="text"
                    className={`flex-1 mx-4 bg-transparent outline-none text-sm font-medium transition-all ${isTodoEditable ? "text-indigo-300 drop-shadow-[0_0_8px_rgba(165,180,252,0.5)]" : "text-zinc-200"
                ...
                <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-all transform translate-x-2 group-hover:translate-x-0">
                    <button
                        className={`px-3 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all ${isTodoEditable
